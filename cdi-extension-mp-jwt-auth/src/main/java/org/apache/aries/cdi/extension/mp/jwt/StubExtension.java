...
import static org.apache.aries.cdi.extension.mp.jwt.StubExtension.EXTENSION_NAME;
...
public class StubExtension extends GeronimoJwtAuthExtension implements BiConsumer<HttpServletRequest, Runnable> {
	...
	void registerMetricsEndpoint(
		...
		beanManager.getEvent().fireAsync(new Ready(adv));
	}
	...
	void registerServices(
		...
		try {
			registerHttpWhiteboardJwtAuthFilter(ready, beanManager, bundleContext);
			registerJaxrsExceptionMapper(beanManager, bundleContext);
			registerJaxrsRequestForwarder(beanManager, bundleContext);
			registerJaxrsRolesAllowed(beanManager, bundleContext);
		}
		catch (Throwable t) {
			ready.getAdv().addDeploymentProblem(t);
		}
	}
	...
	void registerHttpWhiteboardJwtAuthFilter(Ready ready, BeanManager beanManager, BundleContext bundleContext) {
		if (applications.size() > 1) {
			// TODO If and when we have a real JAX-RS extension we should handle this issue there as well, or maybe instead.
			ready.getAdv().addDeploymentProblem(
		...
		if (forceSetup) {
			registerJwtAuthFilter(config, beanManager, bundleContext);
		}
		...
		applications.stream().forEach(app -> {
			...
			properties.put(JAX_RS_EXTENSION_SELECT, new String[] {
				"(&(objectClass=javax.ws.rs.ext.MessageBodyReader)(objectClass=javax.ws.rs.ext.MessageBodyWriter)(osgi.jaxrs.media.type=application/json))",
	...
	void registerJwtAuthFilter(GeronimoJwtAuthConfig config, BeanManager beanManager, BundleContext bundleContext) {
	...
	void registerJaxrsRolesAllowed(BeanManager beanManager, BundleContext bundleContext) {
		Dictionary<String, Object> properties = new Hashtable<>();
	...
	void registerJaxrsRequestForwarder(BeanManager beanManager, BundleContext bundleContext) {
		Dictionary<String, Object> properties = new Hashtable<>();
	...
	void registerJaxrsExceptionMapper(BeanManager beanManager, BundleContext bundleContext) {
		Dictionary<String, Object> properties = new Hashtable<>();
	...
	static Class<?> loadClass(String name) {
		try {
			return ResponseBuilder.class.getClassLoader().loadClass(name);
		}
	...
	static <T> T get(Class<T> clazz, BeanManager beanManager) {
	...
	private static RuntimeException duck(Throwable t) {
		StubExtension.throwsUnchecked(t);
	...
	public static class Ready {
		private final AfterDeploymentValidation adv;

		public Ready(AfterDeploymentValidation adv) {
			this.adv = adv;
		}

		public AfterDeploymentValidation getAdv() {
			return adv;
		}
	}
